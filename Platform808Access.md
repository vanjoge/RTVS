...
RTVS会按照以下规则通过Get请求发送0x9101、0x9201、0x9202等808指令，需要网关实现以下HTTP接口。
    ...
    例如:http://127.0.0.1:8888/WebService/VideoControl?Content=920200090112464004260003030500200820172042
...
| 0x9201,0x9205 |String|  "0"：车辆不在线<br>  "-1"：失败<br> 其他：指令ID | 
...
|  消息ID   | 类型 | 返回数据  |
|  ----  | ----  | ----  |
| 所有  |String|"-1"：失败<br>  "1"：成功（仅指将指令成功发送到网关） | 
...
### 录像列表应答
RTVS向808平台发送查询音视频属性指令后，808平台收到设备应答后，应当将应答结果按下面格式写入redis。

|  类别   | 值  |
|  ----  | ----  |
| 数据类型  | String |
| Key  | OCX_ORDERINFO_[录像列表查询ID] |
