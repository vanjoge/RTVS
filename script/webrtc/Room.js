...
class Room extends EventEmitter
{
	 ...
	 *   mediasoup Router video codecs.
	 */
	static async create({ mediasoupWorker, roomId, rtvsIP, rtvsPort })
	{
	...
	constructor({ roomId, protooRoom, mediasoupRouter, audioLevelObserver, bot, rtvsIP, rtvsPort })
	{
		super();
    ...
    async createRtvsTransport() {
        logger.info('running createRoomForRtvs...');
        ...
        try {
            ...
            let id = this._roomId;
            let displayName = "rtvsupload";
            let rtpCapabilities = this.getRouterRtpCapabilities();
            let device_name = "rtvs";
            let device_version = "1.0.0.1";
            ...
            broadcaster.data.producers.set(audioProducer.id, audioProducer);
            ...
            broadcaster.data.producers.set(videoProducer.id, videoProducer);
            ...
            logger.error('createFfmpegTransport failed:%o', error);
        }
	...
	handleProtooConnection({ peerId, consume, protooWebSocketTransport })
	{
		...
		peer.on('close', () =>
		{
            ...
            //if (this._roomId !== 'ffmpeg_test_room_id') {
	...
	async _handleProtooRequest(peer, request, accept, reject)
	{
		switch (request.method)
		{
			...
			case 'join':
			{
				...
				for (const joinedPeer of joinedPeers)
                {
                    logger.info("joinedPeer:%s", joinedPeer.data.displayName);

					// Create Consumers for existing Producers.
					for (const producer of joinedPeer.data.producers.values())
                    {
                        logger.info("producer ___");
						this._createConsumer(
							{
								consumerPeer : peer,
			...
			case 'createWebRtcTransport':
			{
				...
				if (config.cvconf.onlyTcp)
				{
					webRtcTransportOptions.enableUdp = false;
					webRtcTransportOptions.enableTcp = true;
				}
				else if (config.cvconf.onlyUdp)
				{
	...
	async createFfmpegTransport() {

		logger.info('running createFfmpegTransport...');
	
		try {
			let id = "123";
			let displayName = "ffmpeg_test";
			let rtpCapabilities = this.getRouterRtpCapabilities();
			let device_name = "ffmpeg";
			let device_version = "version";
			...
			this._audioLevelObserver.addProducer({ producerId: audioProducer.id })
				.catch(() => { });
			/**
			this._ffmpegcmd = '.\\ffmpeg -re -v info -stream_loop -1 -i cyx-hukua.mp4 -map 0:a:0 -acodec libopus -ab 128k -ac 2 -ar 48000 ' + 
				'-map 0:v:0 -pix_fmt yuv420p -c:v libvpx -b:v 1000k -deadline realtime -cpu-used 4 -f tee ' +
				'"[select=a:f=rtp:ssrc=11111111:payload_type=101]rtp://192.168.7.153:' + audioRtpPort + '?rtcpport=' + audioRtcpPort +
				'|[select=v:f=rtp:ssrc=22222222: payload_type = 102]rtp://192.168.7.153:' + videoRtpPort + '?rtcpport=' + videoRtcpPort + '"'
			**/
			this._ffmpegcmd = '.\\ffmpeg -re -v info -stream_loop -1 -i dump.flv ' +
				'-map 0:v:0 -pix_fmt yuv420p -c:v copy -cpu-used 4 -f tee ' +
				'"[select=v:f=rtp:ssrc=22222222: payload_type = 102]rtp://192.168.7.153:' + videoRtpPort + '?rtcpport=' + videoRtcpPort + '"'
	
			//打印推流信息
			logger.info(this._ffmpegcmd);
		}
		catch (error) {
			logger.error('createFfmpegTransport failed:%o', error);
		}
